...
def ideal_critical_pressure(
    ...
    '''
    gamma_component = (2 / (upstream_gamma + 1)) ** (upstream_gamma / (upstream_gamma - 1))
...
def ideal_is_choked(
    ...
    critical_press = ideal_critical_pressure(
        upstream_presure,
        upstream_gamma
    )
...
def ideal_orifice_mdot(
    ...
    if  ideal_is_choked(upstream.pressure, upstream.gamma, downstream_pressure):
        ...
        gamma_choked_comp = (2 / (upstream.gamma + 1))**((upstream.gamma + 1) / (upstream.gamma - 1))
        ...
        pressure_ideal1 = (downstream_pressure / upstream.pressure)**(2 / upstream.gamma)
        pressure_ideal2 = (downstream_pressure / upstream.pressure)**((upstream.gamma + 1) / upstream.gamma)
        mdot_kgps = cda * np.sqrt(
