...
def plot_fdm_solution(
    ...
    for i,_ in tqdm(enumerate(time), total=len(time)):
        ...
        for j,t in enumerate(temps):
            ...
            dTdt = conduction_fdm_dT_dt(diffusivity, temps[j + 1], temps[j], temps[j - 1], dx)
            new_t = np_rk4([dTdt, temps[j]],dt)

            if new_t > max_boundary:
                raise ValueError(f"Calcualted Temp: {new_t} > Max Boundary: {max_boundary} at t = {time[i]}")
    ...
    if imperial_results:
        ...
        d = convert(d, 'm', 'in')
        fig.update_layout(
            title = "Imperial Temperature [degF] vs Distance [in]"
        )
    else:
        fig.update_layout(
            title = "SI Temperature [degK] vs Distance [m]"
        )
