...
class DryerOrifice:
    ...
    def __init__(self, cd: float, area: float, fluid: str, beta_ratio=None):
        self.__cd = cd
        self.__area = area
        self.__diameter = circle_diameter_from_area(area)
        self.__cda = cd * area
        self.__cv = convert(self.__cda, "Cda_m2", "Cv")
        self.__fluid = fluid
        self.__beta_ratio = beta_ratio
    ...
    def dict(self, prefix: str = None):
        ...
        return {
            ...
            f"{prefix}diameter [m]": self.__diameter,
    ...
    def mdot(
        ...
        upstream_vapor_pressure = PropsSI(
            "P", "T", upstream_temp, "Q", 1.0, self.__fluid
        )
        ...
        if abs(p_sat - upstream_press) <= 1e-4:
            up_density, up_sp_enthalpy, up_sp_entropy = PropsSI(
                ["D", "HMASS", "SMASS"], "P", upstream_press, "Q", 0.0, self.__fluid
            )
        else:
            up_density, up_sp_enthalpy, up_sp_entropy = PropsSI(
                ["D", "HMASS", "SMASS"],
        ...
        dwn_sp_enthalpy, dwn_density = PropsSI(
            ["HMASS", "D"], "P", downstream_press, "SMASS", up_sp_entropy, self.__fluid
        )

        # Dryer method
        self.__k = dryer.k(upstream_press, upstream_vapor_pressure, downstream_press)

        g_spi = dryer.g_spi(upstream_press, up_density, downstream_press)
        self.__g_spi_mdot = self.cda * g_spi

        g_hem = dryer.g_hem(dwn_density, up_sp_enthalpy, dwn_sp_enthalpy)
