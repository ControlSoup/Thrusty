...
from .. import MIN_RESONABLE_PRESSURE_PA
from .. import pretty_dict
...
class EquilibrumTank:
    def __init__(
        ...
        self.liquid_sat_sp_inenergy = PropsSI('UMASS', 'T', self.temp, 'Q', 0.0, self.fluid)
        self.liquid_sat_density = PropsSI('D', 'T', self.temp, 'Q', 0.0, self.fluid)

        self.vapor_sat_sp_inenergy = PropsSI('UMASS', 'T', self.temp, 'Q', 1.0, self.fluid)
        ...
        self.total_mass = zilliac.eq_22_for_mtotal(
            self.liquid_sat_density,
            self.vapor_sat_density,
            self.x,
            self.volume
        )
    ...
    def from_mass(
        ...
        x = zilliac.eq_23(
            liquid_sat_sp_inenergy,
            liquid_sat_sp_inenergy,
            vapor_sat_sp_inenergy
        )
    ...
    def integrate_state(self, mdot_out: float, dt: float, suppress_warning: bool = False):
        ...
        self.total_inenergy_dot = -mdot_out * PropsSI('HMASS', 'P', self.pressure, 'Q', 0.0, self.fluid)
        ...
        def volume_constraint(temp: float):
            self.liquid_sat_density, self.liquid_sat_sp_inenergy = PropsSI(['D', 'UMASS'], 'T', temp, 'Q', 0.0, self.fluid)
            self.vapor_sat_density, self.vapor_sat_sp_inenergy = PropsSI(['D', 'UMASS'], 'T', temp, 'Q', 1.0, self.fluid)
