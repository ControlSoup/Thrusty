...
for t in tqdm(data.time_array_s):
    ...
    try:
        mdot, is_choked = fluids.real_orifice_mdot(cda, tank.state, STD_ATM_PA, verbose_return=True)
...
for t in tqdm(data.time_array_s):
    ...
    # Mass flow given dp across fitting
    mdot, is_choked = fluids.ideal_orifice_mdot(cda, tank.state, STD_ATM_PA, verbose_return=True)
