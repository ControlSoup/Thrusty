...
orifice: DryerOrifice = DryerOrifice(cd = cd, area = circle_area_from_diameter(orifice_diameter_m), fluid ='N2O')

for t in tqdm(data.data_array):

    # Nitrious
    nitrous_mdot = orifice.mdot(nitrous_tank.pressure, nitrous_tank.temp, convert(150, 'psia', 'Pa') )
    try:
